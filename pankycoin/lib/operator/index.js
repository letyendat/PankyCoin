...
class Operator {
    ...
    loginWallet(secret, password) {
        return R.find((wallet) => { return (wallet.secret == secret && wallet.passwordHash == CryptoUtil.hash(password)); }, this.wallets);
    }    
    ...
    getBalanceForAddress(addressId) {        
        let utxo = this.blockchain.getUnspentTransactionsInBlockChainForAddress(addressId);
