...
class HttpServer {
    constructor(name, node, blockchain, operator, miner) {
        ...
        this.app.post('/operator/wallets/login', (req, res) => {
            let secret = req.body.secret;
            let password = req.body.password;

            let wallet = operator.loginWallet(secret, password);
        ...
        this.app.post('/operator/wallets/:walletId/addresses', (req, res) => {
            ...
            try {
                ...
                let [address, secret] = operator.generateAddressForWallet(walletId);
                res.status(201).send({address:address, secret:secret});
            } catch (ex) {
                if (ex instanceof ArgumentError) throw new HTTPError(400, ex.message, walletId, ex);
                else throw ex;
            }
